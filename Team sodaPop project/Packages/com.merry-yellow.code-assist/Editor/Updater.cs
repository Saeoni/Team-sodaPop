...
namespace Meryel.UnityCodeAssist.Editor
{
    ...
    {
        private const string EditorPrefsKey = "my_uca_update_disabled";
        private const string SessionStateKey = "my_uca_update_checked_before";

        private const string ItchApiUri =
            "https://itch.io/api/1/x/wharf/latest?target=meryel/unity-code-assist&channel_name=asset";

        private const string ItchStoreUri = "https://meryel.itch.io/unity-code-assist";
        private const string AssetStoreUri = "https://assetstore.unity.com/packages/tools/utilities/code-assist-216713";

        private const string VSMarketplaceUri =
            "https://marketplace.visualstudio.com/items?itemName=MerryYellow.UCA-Lite";

        private const string VSCodeMarketplaceUri =
...
#pragma warning disable CS0162
        public static void CheckUpdateSilent()
        {
            ...
            EditorCoroutineUtility.StartCoroutine(GetRequest(ItchApiUri, false), MQTTnetInitializer.Publisher);
        }
#pragma warning restore CS0162

        public static void CheckUpdateForced()
        {
            EditorCoroutineUtility.StartCoroutine(GetRequest(ItchApiUri, true), MQTTnetInitializer.Publisher);
        }
        ...
        private static void DisplayDialog()
        {
            ...
            switch (option)
            {
                ...
                default:
                    Log.Error("Unrecognized option for {Location}.", nameof(DisplayDialog));
                    break;
            }
        }

        private static void Compare(string response, bool isForced)
        {
            var regex = new Regex("\\d+(\\.\\d+){2,4}");
            var match = regex.Match(response);
            if (!match.Success)
            {
                Log.Error("Invalid update response, couldn't parse: {Response}", response);
                return;
            }
            ...
            {
                if (isForced)
                    Log.Information("Unity Code Assist is up to date, version: {Version}", Assister.Version);
                return;
            }
        ...
        private static IEnumerator GetRequest(string uri, bool isForced)
        {
            using (var webRequest = UnityWebRequest.Get(uri))
            {
                ...
                switch (webRequest.result)
                {
                    ...
                    case UnityWebRequest.Result.DataProcessingError:
                        Log.Error("Error while checking new version of UnityCodeAssist " + pages[page] + ": Error: " +
                                  webRequest.error);
                        break;
                    case UnityWebRequest.Result.ProtocolError:
                        Log.Error("Error while checking new version of UnityCodeAssist " + pages[page] +
                                  ": HTTP Error: " + webRequest.error);
                        break;
                    case UnityWebRequest.Result.Success:
                        Log.Debug("Checking new version of UnityCodeAssist " + pages[page] + ": Received: " +
