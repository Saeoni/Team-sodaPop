...
namespace Meryel.UnityCodeAssist.Editor
{
    ...
    public static class Cleanup
    {
        public static bool DoCleanup()
        {
            ...
            {
                succeed = false;
                Log.Error(ex, "DoCleanup failed at {Destination}", destination);
            }
        ...
        private static void DeleteFileAndItsMeta(string filePath)
        {
            ...
            {
                Log.Debug("Deleting file {File}", filePath);
                //File.Delete(filePath);
                DeleteFileAux(filePath);
                Log.Debug("Deleted file {File} {Exists}", filePath, File.Exists(filePath));
            }

            var metaFilePath = filePath + ".meta";
            if (File.Exists(metaFilePath))
            {
                Log.Debug("Deleting meta file {File}", metaFilePath);
                //File.Delete(metaFilePath);
                DeleteFileAux(metaFilePath);
                Log.Debug("Deleted file {File} {Exists}", metaFilePath, File.Exists(metaFilePath));
            }
        }
        ...
        private static void DeleteDirectoryAndItsMeta(string directoryPath)
        {
            ...
            {
                Log.Debug("Deleting directory {Dir}", directoryPath);
                Directory.Delete(directoryPath);
                Log.Debug("Deleted directory {Dir} {Exists}", directoryPath, IsDirectoryExistsAndEmpty(directoryPath));

                var metaFilePath = directoryPath + ".meta";
                if (File.Exists(metaFilePath))
                {
                    Log.Debug("Deleting directory meta file {File}", metaFilePath);
                    //File.Delete(metaFilePath);
                    DeleteFileAux(metaFilePath);
                    Log.Debug("Deleted directory meta file {File} {Exists}", metaFilePath, File.Exists(metaFilePath));
                }
            }
        }
        ...
        /// <returns></returns>
        private static bool SetEveryoneAccessToDirectory(string dirName, out string _lastError)
        {
            try
            {
        ...
        public static int GetCSharpVersionFromUnityProjectVersionFile(string? solutionDirectory)
        {
            ...
            var projectVersionFilePath = Path.Combine(solutionDirectory, "ProjectSettings/ProjectVersion.txt");
            ...
            try
            {
                readText = File.ReadAllLines(projectVersionFilePath);
            ...
            {
                Log.Error(ex, "Project version file parsing error {FirstLine}", readText?.FirstOrDefault());
                return -1;
            }

            if (version == null)
            {
                Log.Error("Parsed project version is null");
                return -1;
            }
            ...
            Log.Error("Parsed project version is unknown {Version}", version);
            return -1;
        }
        ...
        private static void Cleanup1(string destination)
        {
            ...
            if (Directory.Exists(oldBinariesDirectory))
            {
                ...
                Log.Debug("Old binaries directory exists at {Location}", oldBinariesDirectory);
