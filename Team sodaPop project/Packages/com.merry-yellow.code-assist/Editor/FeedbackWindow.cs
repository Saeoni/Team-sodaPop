...
namespace Meryel.UnityCodeAssist.Editor
{
    public class FeedbackWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            var errorCount = ELogger.GetErrorCountInInternalLog();
            var warningCount = ELogger.GetWarningCountInInternalLog();
            var logContent = ELogger.GetInternalLogContent();
            ...
            {
                var filePath = ELogger.FilePath;
                Process.Start(filePath);
            }

            if (GUILayout.Button("View Visual Studio full log"))
            {
                var filePath = ELogger.VSFilePath;
                Process.Start(filePath);
            }
