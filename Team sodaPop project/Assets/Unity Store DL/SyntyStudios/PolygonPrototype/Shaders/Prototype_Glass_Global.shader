...
Shader "SyntyStudios/Prototype_Glass_Global"
{
    ...
    SubShader
    {
        ...
        inline float4 TriplanarSamplingCF(sampler2D topTexMap, sampler2D midTexMap, sampler2D botTexMap,
            float3 worldPos, float3 worldNormal, float falloff, float tilling, float3 normalScale, float3 index)
        {
        ...
        void surf(Input i, inout SurfaceOutputStandardSpecular o)
        {
            ...
            float4 triplanar6 = TriplanarSamplingCF(_Grid, _Grid, _Grid, ase_worldPos, ase_worldNormal, _Falloff,
                       _GridScale, float3(1, 1, 1), float3(0, 0, 0));
